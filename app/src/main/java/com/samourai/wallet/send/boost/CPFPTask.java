            ...
            try {
                ...
                if (utxo != null) {
                    ...
                    if (FormatsUtil.getInstance().isValidBech32(addr)) {
                        ownReceiveAddr = AddressFactory.getInstance(activity).getBIP84(AddressFactory.RECEIVE_CHAIN).getRight().getBech32AsString();
                    } else if (Address.fromBase58(SamouraiWallet.getInstance().getCurrentNetworkParams(), addr).isP2SHAddress()) {
                        ownReceiveAddr = AddressFactory.getInstance(activity).getBIP49(AddressFactory.RECEIVE_CHAIN).getRight().getAddressAsString();
                    } else {
                        ownReceiveAddr = AddressFactory.getInstance(activity).get(AddressFactory.RECEIVE_CHAIN).getRight().getAddressString();
                    }
