...
public class PaynymTxListAdapter extends RecyclerView.Adapter<PaynymTxListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (tx.getAmount() < 0.0) {
            ...
            holder.icon.setColorFilter(android.R.color.white);
            holder.amount.setText("You sent ".concat(this.getBTCDisplayAmount(_amount)).concat(" BTC"));
        } else {
            holder.icon.setImageResource(R.drawable.out_going_tx_whtie_arrow);
            holder.amount.setText("You received ".concat(this.getBTCDisplayAmount(_amount)).concat(" BTC"));
