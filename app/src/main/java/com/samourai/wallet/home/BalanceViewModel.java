...
public class BalanceViewModel extends AndroidViewModel {
    ...
    public BalanceViewModel(@NonNull Application application) {
        ...
        try {
            String response = PayloadUtil.getInstance(application).deserializeMultiAddr().toString();
//            Log.i(TAG, "BalanceViewModel: ".concat(response));

            parseXPUB(new JSONObject(response));
            Log.i(TAG, "BalanceViewModel: ");
    ...
    public LiveData<List<Tx>> getTxs() {
        if (txs.getValue() == null) {
            txs = new MutableLiveData<>();
        }
        return txs;
    }
