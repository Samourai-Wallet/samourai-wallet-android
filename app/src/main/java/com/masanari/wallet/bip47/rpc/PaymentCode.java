package com.masanari.wallet.bip47.rpc;
...
public class PaymentCode {
    ...
    private static final int MASANARI_FEATURE_BYTE = 79;
    private static final int MASANARI_SEGWIT_BIT = 0;
    ...
    public HD_Address addressAt(int idx) throws AddressFormatException {
        return new BIP47Account(MasanariWallet.getInstance().getCurrentNetworkParams(), strPaymentCode).addressAt(idx);
    }
    ...
    public String makeMasanariPaymentCode() throws AddressFormatException {

        byte[] payload = getPayload();
        // set bit0 = 1 in 'Masanari byte' for segwit. Can send/receive P2PKH, P2SH-P2WPKH, P2WPKH (bech32)
        payload[MASANARI_FEATURE_BYTE] = setBit(payload[MASANARI_FEATURE_BYTE], MASANARI_SEGWIT_BIT);
        byte[] payment_code = new byte[PAYLOAD_LEN + 1];
        // add version byte
        payment_code[0] = (byte)0x47;
        System.arraycopy(payload, 0, payment_code, 1, payload.length);
