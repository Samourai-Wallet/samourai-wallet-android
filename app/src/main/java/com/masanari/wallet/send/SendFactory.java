    ...
    public Transaction signTransactionForSweep(Transaction unsignedTx, PrivKeyReader privKeyReader)    {
        ...
        for (TransactionInput input : unsignedTx.getInputs()) {
            ...
            try {
                ...
                ECKey ecKey = null;
                try {
                    DumpedPrivateKey pk = new DumpedPrivateKey(MasanariWallet.getInstance().getCurrentNetworkParams(), privKeyReader.getKey().getPrivateKeyAsWiF(MasanariWallet.getInstance().getCurrentNetworkParams()));
                    ecKey = pk.getKey();
//                    Log.i("SendFactory", "ECKey address:" + ecKey.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString());
...
/*
            if(value.compareTo(MasanariWallet.bDust) < 1)    {
...
*/
            ...
            if(!FormatsUtil.getInstance().isValidBitcoinAddress(toAddress) && FormatsUtil.getInstance().isValidBIP47OpReturn(toAddress))    {
                ...
                output = new TransactionOutput(MasanariWallet.getInstance().getCurrentNetworkParams(), null, Coin.valueOf(0L), toOutputScript.getProgram());
            }
                ...
                toOutputScript = ScriptBuilder.createOutputScript(org.bitcoinj.core.Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), toAddress));
                output = new TransactionOutput(MasanariWallet.getInstance().getCurrentNetworkParams(), null, Coin.valueOf(value.longValue()), toOutputScript.getProgram());
            }
        ...
        for(MyTransactionOutPoint outPoint : unspent) {
            ...
            MyTransactionInput input = new MyTransactionInput(MasanariWallet.getInstance().getCurrentNetworkParams(), null, new byte[0], outPoint, outPoint.getTxHash().toString(), outPoint.getTxOutputN());
            if(PrefsUtil.getInstance(context).getValue(PrefsUtil.RBF_OPT_IN, false) == true)    {
                input.setSequenceNumber(MasanariWallet.RBF_SEQUENCE_NO);
            }
            inputs.add(input);
        }
    ...
    private synchronized Transaction signTransaction(Transaction transaction, HashMap<String,ECKey> keyBag) throws ScriptException {
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            if(Bech32Util.getInstance().isBech32Script(script))    {
                ...
                address = new Script(connectedPubKeyScript).getToAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString();
            }
            ...
            if(FormatsUtil.getInstance().isValidBech32(address) || Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), address).isP2SHAddress())    {
                ...
                final SegwitAddress segwitAddress = new SegwitAddress(key.getPubKey(), MasanariWallet.getInstance().getCurrentNetworkParams());
...
//                System.out.println("to address from script:" + scriptPubKey.getToAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString());
...
//                System.out.println("script code:" + Hex.toHexString(scriptCode.getProgram()));
    ...
    public Triple<ArrayList<MyTransactionOutPoint>, ArrayList<TransactionOutput>, ArrayList<UTXO>> boltzmannSet(List<UTXO> utxos, BigInteger spendAmount, String address, List<MyTransactionOutPoint> firstPassOutpoints) {
        ...
        if(PrefsUtil.getInstance(context).getValue(PrefsUtil.USE_LIKE_TYPED_CHANGE, true) == true)    {
            ...
            else if(Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), utxoAddress).isP2SHAddress())   {
                changeType = 49;
            }
            else    {
                changeType = 44;
        ...
        BigInteger bDust = firstPassOutpoints == null ? BigInteger.ZERO : MasanariWallet.bDust;
        ...
        if(changeDue.subtract(biFee).compareTo(MasanariWallet.bDust) > 0)    {
        ...
        try {
            ...
            if(FormatsUtil.getInstance().isValidBech32(_address))   {
                ...
                outputScript = ScriptBuilder.createOutputScript(org.bitcoinj.core.Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), _address));
                txSpendOutput = new TransactionOutput(MasanariWallet.getInstance().getCurrentNetworkParams(), null, Coin.valueOf(spendAmount.longValue()), outputScript.getProgram());
            }
            ...
            if(FormatsUtil.getInstance().isValidBech32(changeAddress))    {
                ...
                outputScript = ScriptBuilder.createOutputScript(org.bitcoinj.core.Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), changeAddress));
                txChangeOutput = new TransactionOutput(MasanariWallet.getInstance().getCurrentNetworkParams(), null, Coin.valueOf(changeDue.longValue()), outputScript.getProgram());
            }
