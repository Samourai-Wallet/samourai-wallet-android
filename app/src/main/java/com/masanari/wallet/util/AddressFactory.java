...
public class AddressFactory {
    ...
    public HD_Address get(int chain)	{
        ...
        try	{
            ...
            if(hdw != null)    {
                idx = HD_WalletFactory.getInstance(context).get().getAccount(MasanariWallet.MASANARI_ACCOUNT).getChain(chain).getAddrIdx();
                addr = HD_WalletFactory.getInstance(context).get().getAccount(MasanariWallet.MASANARI_ACCOUNT).getChain(chain).getAddressAt(idx);
                if(chain == RECEIVE_CHAIN && canIncReceiveAddress(MasanariWallet.MASANARI_ACCOUNT))	{
                    HD_WalletFactory.getInstance(context).get().getAccount(MasanariWallet.MASANARI_ACCOUNT).getChain(chain).incAddrIdx();
...
//        try	{
            ...
            if(hdw != null)    {
                idx = BIP49Util.getInstance(context).getWallet().getAccount(MasanariWallet.MASANARI_ACCOUNT).getChain(chain).getAddrIdx();
                addr = BIP49Util.getInstance(context).getWallet().getAccount(MasanariWallet.MASANARI_ACCOUNT).getChain(chain).getAddressAt(idx);
                p2shp2wpkh = new SegwitAddress(addr.getPubKey(), MasanariWallet.getInstance().getCurrentNetworkParams());
                if(chain == RECEIVE_CHAIN && canIncBIP49ReceiveAddress(idx))	{
                    BIP49Util.getInstance(context).getWallet().getAccount(MasanariWallet.MASANARI_ACCOUNT).getChain(chain).incAddrIdx();
//                    PayloadUtil.getInstance(context).saveWalletToJSON(new CharSequenceX(AccessFactory.getInstance(context).getGUID() + AccessFactory.getInstance(context).getPIN()));
                }
            }
//        }
...
//        try	{
        ...
        if(hdw != null)    {
            idx = BIP84Util.getInstance(context).getWallet().getAccount(MasanariWallet.MASANARI_ACCOUNT).getChain(chain).getAddrIdx();
            addr = BIP84Util.getInstance(context).getWallet().getAccount(MasanariWallet.MASANARI_ACCOUNT).getChain(chain).getAddressAt(idx);
            p2wpkh = new SegwitAddress(addr.getPubKey(), MasanariWallet.getInstance().getCurrentNetworkParams());
            if(chain == RECEIVE_CHAIN && canIncBIP84ReceiveAddress(idx))	{
                BIP84Util.getInstance(context).getWallet().getAccount(MasanariWallet.MASANARI_ACCOUNT).getChain(chain).incAddrIdx();
//                    PayloadUtil.getInstance(context).saveWalletToJSON(new CharSequenceX(AccessFactory.getInstance(context).getGUID() + AccessFactory.getInstance(context).getPIN()));
            }
        }
//        }
