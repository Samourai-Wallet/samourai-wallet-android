...
import com.masanari.wallet.segwit.bech32.Bech32;
...
public class FormatsUtil {
	...
	public boolean isValidBitcoinAddress(final String address) {
		...
		if((!MasanariWallet.getInstance().isTestNet() && address.toLowerCase().startsWith("bc")) ||
				(MasanariWallet.getInstance().isTestNet() && address.toLowerCase().startsWith("tb")))	{
			...
			try {
				addr = new Address(MasanariWallet.getInstance().getCurrentNetworkParams(), address);
				if(addr != null) {
					ret = true;
				}
			}
