...
//    public final static String HEX_COMPRESSED = "hex_c";
    ...
    public String getFormat() throws Exception {
        ...
        if(MasanariWallet.getInstance().isTestNet() && strPrivKey.toString().matches("^[c][1-9A-HJ-NP-Za-km-z]{51}$")) {
            return WIF_COMPRESSED;
        }
        // 51 characters base58, always starts with a '9'
        else if(MasanariWallet.getInstance().isTestNet() && strPrivKey.toString().matches("^9[1-9A-HJ-NP-Za-km-z]{50}$")) {
            return WIF_UNCOMPRESSED;
        }
        // 52 characters, always starts with 'K' or 'L'
        else if(!MasanariWallet.getInstance().isTestNet() && strPrivKey.toString().matches("^[LK][1-9A-HJ-NP-Za-km-z]{51}$")) {
            return WIF_COMPRESSED;
        }
        // 51 characters base58, always starts with a '5'
        else if(!MasanariWallet.getInstance().isTestNet() && strPrivKey.toString().matches("^5[1-9A-HJ-NP-Za-km-z]{50}$")) {
            return WIF_UNCOMPRESSED;
        }
...
//                strPrivKey.toString().matches("^S[1-9A-HJ-NP-Za-km-z]{29}$") ||
    ...
    public ECKey getKey() throws Exception {
        ...
        if(format.equals(WIF_COMPRESSED) || format.equals(WIF_UNCOMPRESSED)) {
            DumpedPrivateKey pk = DumpedPrivateKey.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), strPrivKey.toString());
            return pk.getKey();
        }
    ...
    private ECKey parseBIP38(String encryptedKey, CharSequenceX password)  {
        ...
        try {
            BIP38PrivateKey bip38 = new BIP38PrivateKey(MasanariWallet.getInstance().getCurrentNetworkParams(), encryptedKey);
