...
public class MessageSignUtil {
    ...
    public boolean verifySignedMessage(String address, String strMessage, String strSignature) throws SignatureException {
        ...
        if(ecKey != null)   {
            return ecKey.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString().equals(address);
        }
        else    {
            return false;
        }
    ...
    private String signMessageArmored(ECKey key, String strMessage) {
        ...
        if(sig != null)    {
            ...
            ret += "Address: " + key.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString() + "\n\n";
            ret += sig;
            ret += "\n";
