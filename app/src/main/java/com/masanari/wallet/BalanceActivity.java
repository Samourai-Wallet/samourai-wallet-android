...
import com.masanari.wallet.JSONRPC.PoW;
...
import com.masanari.wallet.api.Tx;
...
import com.masanari.wallet.send.RBFSpend;
import com.masanari.wallet.send.RBFUtil;
import com.masanari.wallet.send.SendFactory;
import com.masanari.wallet.send.SuggestedFee;
import com.masanari.wallet.send.SweepUtil;
import com.masanari.wallet.send.UTXO;
import com.masanari.wallet.send.PushTx;
import com.masanari.wallet.service.RefreshService;
import com.masanari.wallet.service.WebSocketService;
import com.masanari.wallet.util.AddressFactory;
import com.masanari.wallet.util.AppUtil;
import com.masanari.wallet.util.BlockExplorerUtil;
import com.masanari.wallet.util.CharSequenceX;
import com.masanari.wallet.util.DateUtil;
...
import com.masanari.wallet.util.PrivKeyReader;
import com.masanari.wallet.util.TimeOutUtil;
import com.masanari.wallet.util.TorUtil;
import com.masanari.wallet.util.TypefaceUtil;
...
public class BalanceActivity extends Activity {
    ...
    public static final String ACTION_INTENT = "com.masanari.wallet.BalanceFragment.REFRESH";
...
//                    @Override
    ...
    public static final String DISPLAY_INTENT = "com.masanari.wallet.BalanceFragment.DISPLAY";
...
//                            @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(MasanariWallet.getInstance().isTestNet())    {
        ...
        actionBIP47.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View arg0) {
                Intent intent = new Intent(BalanceActivity.this, com.masanari.wallet.bip47.BIP47Activity.class);
                startActivity(intent);
            }
        });
...
//        LocalBroadcastManager.getInstance(BalanceActivity.this).registerReceiver(receiver, filter);
        ...
        Intent intent = new Intent("com.masanari.wallet.MainActivity2.RESTART_SERVICE");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_settings) {
            ...
            if(MasanariWallet.getInstance().hasPassphrase(BalanceActivity.this))    {
                try {
                    if(HD_WalletFactory.getInstance(BalanceActivity.this).get() != null && MasanariWallet.getInstance().hasPassphrase(BalanceActivity.this))    {
                        doBackup();
                    }
                    else    {
    ...
    private void doSupport()	{
        Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/ivohuisjes/masanari-wallet-android/issues"));
        startActivity(intent);
    }
                ...
                AlertDialog.Builder dlg = new AlertDialog.Builder(BalanceActivity.this)
                        ...
                        .setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int whichButton) {
                                ...
                                try {
                                    BIP38PrivateKey bip38 = new BIP38PrivateKey(MasanariWallet.getInstance().getCurrentNetworkParams(), data);
                                    ...
                                    if(ecKey != null && ecKey.hasPrivKey()) {
                                        ...
                                        Toast.makeText(BalanceActivity.this, pvr.getKey().toAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString(), Toast.LENGTH_SHORT).show();
                    ...
                    .setSingleChoiceItems(export_methods, 0, new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    if (which == 0) {
                                        ...
                                        email.putExtra(Intent.EXTRA_SUBJECT, "Masanari Wallet backup");
    ...
    private void displayBalance() {
        ...
        long balance = 0L;
        if(MasanariWallet.getInstance().getShowTotalBalance())    {
            if(MasanariWallet.getInstance().getCurrentSelectedAccount() == 0)    {
                ...
                if(APIFactory.getInstance(BalanceActivity.this).getXpubAmounts().size() > 0)    {
                    try    {
                        if(APIFactory.getInstance(BalanceActivity.this).getXpubAmounts().get(HD_WalletFactory.getInstance(BalanceActivity.this).get().getAccount(MasanariWallet.getInstance().getCurrentSelectedAccount() - 1).xpubstr()) != null)    {
                            balance = APIFactory.getInstance(BalanceActivity.this).getXpubAmounts().get(HD_WalletFactory.getInstance(BalanceActivity.this).get().getAccount(MasanariWallet.getInstance().getCurrentSelectedAccount() - 1).xpubstr());
                        }
                    }
                    catch(IOException ioe)    {
                        ;
            ...
            if(APIFactory.getInstance(BalanceActivity.this).getXpubAmounts().size() > 0)    {
                try    {
                    if(APIFactory.getInstance(BalanceActivity.this).getXpubAmounts().get(HD_WalletFactory.getInstance(BalanceActivity.this).get().getAccount(MasanariWallet.getInstance().getCurrentSelectedAccount()).xpubstr()) != null)    {
                        balance = APIFactory.getInstance(BalanceActivity.this).getXpubAmounts().get(HD_WalletFactory.getInstance(BalanceActivity.this).get().getAccount(MasanariWallet.MASANARI_ACCOUNT).xpubstr());
                    }
                }
                catch(IOException ioe)    {
                    ;
            ...
            if(txObj.has("inputs") && txObj.has("outputs"))    {
                ...
                try {
                    ...
                    for(int i = 0; i < inputs.length(); i++)   {
                        if(inputs.getJSONObject(i).has("outpoint") && inputs.getJSONObject(i).getJSONObject("outpoint").has("scriptpubkey"))    {
                            ...
                            if(Bech32Util.getInstance().isBech32Script(scriptpubkey))    {
                                ...
                                address = script.getToAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString();
                            }
                            if(FormatsUtil.getInstance().isValidBech32(address))    {
                                p2wpkh++;
                            }
                            else if(Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), address).isP2SHAddress())    {
                                p2sh_p2wpkh++;
                            }
                            else    {
                                p2pkh++;
                    ...
                    if(utxo != null)    {
                        ...
                        else if(Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), addr).isP2SHAddress())    {
                        ...
                        if(totalAmount < (cpfpFee.longValue() + remainingFee)) {
                            ...
                            for(UTXO _utxo : utxos)   {
                                ...
                                if(totalAmount > (cpfpFee.longValue() + remainingFee + MasanariWallet.bDust.longValue())) {
                                    break;
                                }
                            }
                            if(totalAmount < (cpfpFee.longValue() + remainingFee + MasanariWallet.bDust.longValue())) {
                        ...
                        if(amount < MasanariWallet.bDust.longValue())    {
    ...
    private class RBFTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(final String... params) {
            ...
            final Transaction tx = new Transaction(MasanariWallet.getInstance().getCurrentNetworkParams(), Hex.decode(rbf.getSerializedTx()));
                ...
                try {
                    ...
                    if(total_change > remainder)    {
                        for(TransactionOutput output : txOutputs)   {
                            ...
                            Address _p2sh = output.getAddressFromP2SH(MasanariWallet.getInstance().getCurrentNetworkParams());
                            Address _p2pkh = output.getAddressFromP2PKHScript(MasanariWallet.getInstance().getCurrentNetworkParams());
                            try {
                                if((Bech32Util.getInstance().isBech32Script(scriptPubKey) && rbf.getChangeAddrs().contains(Bech32Util.getInstance().getAddressFromScript(scriptPubKey))) || (_p2sh != null && rbf.getChangeAddrs().contains(_p2sh.toString())) || (_p2pkh != null && rbf.getChangeAddrs().contains(_p2pkh.toString())))    {
                                    if(output.getValue().longValue() >= (remainder + MasanariWallet.bDust.longValue()))    {
                    ...
                    for(TransactionInput input : txInputs) {
                        MyTransactionInput _input = new MyTransactionInput(MasanariWallet.getInstance().getCurrentNetworkParams(), null, new byte[0], input.getOutpoint(), input.getOutpoint().getHash().toString(), (int)input.getOutpoint().getIndex());
                        _input.setSequenceNumber(MasanariWallet.RBF_SEQUENCE_NO);
                        _inputs.add(_input);
                    ...
                    if(remainder > 0L)    {
                        ...
                        for(UTXO _utxo : utxos)   {
                            ...
                            if(selectedAmount >= (_remainingFee + MasanariWallet.bDust.longValue())) {
                                break;
                            }
                        }
                        long extraChange = 0L;
                        ...
                        if(outputs.length() == 1 && extraChange > 0L)    {
                            try {
                                ...
                                Script toOutputScript = ScriptBuilder.createOutputScript(org.bitcoinj.core.Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), change_address));
                                TransactionOutput output = new TransactionOutput(MasanariWallet.getInstance().getCurrentNetworkParams(), null, Coin.valueOf(extraChange), toOutputScript.getProgram());
                        ...
                        // parent tx had change output
                            ...
                            for(TransactionOutput output : txOutputs)   {
                                ...
                                if(_addr == null)    {
                                    Address _address = output.getAddressFromP2PKHScript(MasanariWallet.getInstance().getCurrentNetworkParams());
                                    if(_address == null)    {
                                        _address = output.getAddressFromP2SH(MasanariWallet.getInstance().getCurrentNetworkParams());
                                    }
                        ...
                        for(UTXO _utxo : selectedUTXO)    {
                            ...
                            for(MyTransactionOutPoint outpoint : _utxo.getOutpoints()) {
                                ...
                                MyTransactionInput _input = new MyTransactionInput(MasanariWallet.getInstance().getCurrentNetworkParams(), null, new byte[0], outpoint, outpoint.getTxHash().toString(), outpoint.getTxOutputN());
                                _input.setSequenceNumber(MasanariWallet.RBF_SEQUENCE_NO);
                                _inputs.add(_input);
                                ...
                                if(path != null)    {
                                    ...
                                    else if(Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), outpoint.getAddress()) != null && Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), outpoint.getAddress()).isP2SHAddress())    {
        ...
        private Transaction signTx(Transaction tx)    {
            ...
            for(String outpoint : keys.keySet())   {
                ...
                if(s.length == 4)    {
                    ...
                    DumpedPrivateKey pk = new DumpedPrivateKey(MasanariWallet.getInstance().getCurrentNetworkParams(), strPrivKey);
                    ecKey = pk.getKey();
                }
                else if(s.length == 2)    {
                    try {
...
//                Log.i("BalanceActivity", "ECKey address from ECKey:" + ecKey.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString());
            ...
            for (int i = 0; i < inputs.size(); i++) {
                ...
                if(inputs.get(i).getValue() != null || keyBag49.containsKey(inputs.get(i).getOutpoint().toString()) || keyBag84.containsKey(inputs.get(i).getOutpoint().toString()))    {
                    if(keyBag84.containsKey(inputs.get(i).getOutpoint().toString()))    {
                        ...
                        SegwitAddress segwitAddress = new SegwitAddress(ecKey.getPubKey(), MasanariWallet.getInstance().getCurrentNetworkParams());
                    ...
                    address = ecKey.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString();
                }
                ...
                if(inputs.get(i).getValue() != null || keyBag49.containsKey(inputs.get(i).getOutpoint().toString()) || keyBag84.containsKey(inputs.get(i).getOutpoint().toString()))    {
                    ...
                    final SegwitAddress segwitAddress = new SegwitAddress(ecKey.getPubKey(), MasanariWallet.getInstance().getCurrentNetworkParams());
                    ...
                    if(!FormatsUtil.getInstance().isValidBech32(address) && Address.fromBase58(MasanariWallet.getInstance().getCurrentNetworkParams(), address).isP2SHAddress())    {
                    ...
                    Log.i("BalanceActivity", "ECKey address from keyBag:" + ecKey.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString());

                    Log.i("BalanceActivity", "script:" + ScriptBuilder.createOutputScript(ecKey.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams())));
                    Log.i("BalanceActivity", "script:" + Hex.toHexString(ScriptBuilder.createOutputScript(ecKey.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams())).getProgram()));
                    TransactionSignature sig = tx.calculateSignature(i, ecKey, ScriptBuilder.createOutputScript(ecKey.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams())).getProgram(), Transaction.SigHash.ALL, false);
...
//                    Log.d("BalanceActivity", obj.toString());
                    String res = com.masanari.wallet.bip47.paynym.WebUtil.getInstance(BalanceActivity.this).postURL("application/json", null, com.masanari.wallet.bip47.paynym.WebUtil.PAYNYM_API + "api/v1/token", obj.toString());
...
//                        Log.d("BalanceActivity", "nym/add:" + obj.toString());
                        res = com.masanari.wallet.bip47.paynym.WebUtil.getInstance(BalanceActivity.this).postURL("application/json", token, com.masanari.wallet.bip47.paynym.WebUtil.PAYNYM_API + "api/v1/nym/add", obj.toString());
//                        Log.d("BalanceActivity", res);
