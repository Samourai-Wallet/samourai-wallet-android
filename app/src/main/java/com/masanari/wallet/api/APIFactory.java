package com.masanari.wallet.api;
...
public class APIFactory	{
    ...
    private synchronized JSONObject getXPUB(String[] xpubs, boolean parse) {
        ...
        String _url = MasanariWallet.getInstance().isTestNet() ? WebUtil.MASANARI_API2_TESTNET : WebUtil.MASANARI_API2;
        ...
        try {
            ...
            if(AddressFactory.getInstance(context).xpub2account().get(xpub) != null || xpub.equals(BIP49Util.getInstance(context).getWallet().getAccount(0).ypubstr()) || xpub.equals(BIP84Util.getInstance(context).getWallet().getAccount(0).zpubstr()))    {
                ...
                if(ecKey != null && ecKey.hasPrivKey())    {
                    ...
                    switch(purpose)    {
                        case 49:
                            SegwitAddress p2shp2wpkh = new SegwitAddress(ecKey.getPubKey(), MasanariWallet.getInstance().getCurrentNetworkParams());
        ...
        if(jsonObject != null)  {
            ...
            if(jsonObject.has("inputs"))    {
                ...
                if(inArray.length() > 0)    {
                    ...
                    Log.i("APIFactory", "address from script:" + pKey.toAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString());
//                        Log.i("APIFactory", "uncompressed public key from script:" + Hex.toHexString(pKey.decompress().getPubKey()));
                    ...
                    if(((JSONObject)inArray.get(0)).has("outpoint"))    {
                        ...
                        TransactionOutPoint outPoint = new TransactionOutPoint(MasanariWallet.getInstance().getCurrentNetworkParams(), idx, txHash);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if(!AppUtil.getInstance(context).isOfflineMode()) {
                    ...
                    try {
                        String response = WebUtil.getInstance(context).getURL(WebUtil.MASANARI_API_CHECK);
