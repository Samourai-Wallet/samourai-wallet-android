...
import com.masanari.wallet.BalanceActivity;
...
public class RicochetActivity extends Activity {
    ...
    private boolean masanariFeeViaBIP47 = false;
            ...
            try {
                ...
                while(itr.hasNext()){
                    ...
                    if(!txSeen)    {
                        ...
                        if(jObj.has("masanariFeeViaBIP47"))    {
                            masanariFeeViaBIP47 = jObj.getBoolean("masanariFeeViaBIP47");
                        }
...
//                            Log.d("RicochetActivity", "seq:" + seq + ":" + dest);
                        ...
                        if(txs.length >= 5 && dests.length >= 5)    {
                            ...
                            while(i < txs.length)   {
                                ...
                                String response = PushTx.getInstance(RicochetActivity.this).masanari(txs[i]);
                                    ...
                                    if(i == (txs.length - 1))    {
                                        ...
                                        if(masanariFeeViaBIP47)    {
                                            for(int j = 0; j < 4; j++)   {
                                                ...
                                                PaymentAddress paymentAddress = BIP47Util.getInstance(RicochetActivity.this).getSendAddress(pcode, BIP47Meta.getInstance().getOutgoingIdx(BIP47Meta.strMasanariDonationPCode));
                                                String strAddress = paymentAddress.getSendECKey().toAddress(MasanariWallet.getInstance().getCurrentNetworkParams()).toString();
                                                BIP47Meta.getInstance().getPCode4AddrLookup().put(strAddress, BIP47Meta.strMasanariDonationPCode);
                                                BIP47Meta.getInstance().inc(BIP47Meta.strMasanariDonationPCode);
                                            }
                                        }
